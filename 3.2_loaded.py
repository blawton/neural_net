...
xtrains = xtrain[0:batches*batch_size,:,:,:]
ytrains = ytrain[0:batches*batch_size]
xtests = xtest[0:tests*batch_size,:,:,:]
ytests = ytest[0:tests*batch_size]
...
with sess.as_default():
    for test in range(tests):
        testresults = tpred.eval(feed_dict={X: xtrains[test*batch_size:test*batch_size + batch_size, :, :, :]})
        matches = (np.argmax(testresults, axis=1) == ytrains[test*batch_size:test*batch_size + batch_size])
        print(np.argmax(testresults, axis=1))
        print(ytrains[test*batch_size:test*batch_size + batch_size])
    ...
    for epoch in range(epochs):
        ...
        for batch in range(batches):
            _, currentloss = sess.run([mytrain_op, tloss], feed_dict={X: xtrains[batch*batch_size:batch*batch_size + batch_size, :, :, :],
                                      Y: ytrains[batch*batch_size:batch*batch_size + batch_size]})
